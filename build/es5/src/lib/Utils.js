...
export const createGlobalResponse = (methods, logicFunction) => {
  ...
  methods.forEach(method => {
    response[method] = (callback, requiereParams = [], filter = {}) => logicFunction({
      method,
      callback,
      requiereParams,
      filter
...
export const getProptypes = component => {
  ...
  let obj = component;
  do {
    const l = Object.getOwnPropertyNames(obj).concat(Object.getOwnPropertySymbols(obj).map(s => s.toString())).sort().filter((p, i, arr) => typeof obj[p] === 'function' && reservedMethods.indexOf(p) === -1 && (i == 0 || p !== arr[i - 1]) && props.indexOf(p) === -1);
