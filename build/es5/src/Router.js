...
import { print, setError, setSlogan, createGlobalResponse, getProptypes, getParentName, getBasePath, getFnParamNames, avalaibleHeader } from './lib/Utils';
...
class Router {
  constructor(options = {}) {
    ...
    this.response = createGlobalResponse(this.avalaibleMethods, this.methods);
    // Path to use
    this.path = '';
    this.slogan = '[SWRest]';
    // Init
  ...
  setAdvanceRoute(component, test = false) {
    ...
    methods.forEach(key => {
      ...
      if (test) print('info', `Possible route: ${url} (Method: ${method})`);else {
  ...
  methods({
    ...
    this.server[method](path || this.path, async (req, res) => {
      // Verificamos header
      ...
      await Listen.Request(filter.authRequired, req.get('X-Auth-Token'), method, callback, req, res, requiereParams,
      // Login callback
      this.checkLogin);
    });
    // Respondemos object
