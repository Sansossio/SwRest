...
class Listen {
  ...
  static generateResponse(callback, res, params, method) {
    ...
    const response =
      callback(paramsFixed, method, ...Object.values(paramsUrl)) || setError('Callback need a return');
    // Enviamos
    ...
    // Respondemos
    if (Listen.jsonParse) res.status(status).json(send);
    else res.send(send);
  }
  static Params(method, req) {
    ...
    const response = Object.assign(
      Object.keys(query).length > 0 ? { url: query } : {},
      { query: req.params },
      params,
